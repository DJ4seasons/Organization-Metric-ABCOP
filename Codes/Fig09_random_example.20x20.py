...
def main():
    ...
    for k in range(nSample):
        rg.shuffle(xx)
        rg.shuffle(yy)
        arr1[k,xx[:max_cells],yy[:max_cells]]=1
        
    ### Case2: One Gaussian normal distribution on center of domain
    arr2= np.zeros([nSample,ny,nx])
    rg= np.random.default_rng(rg_seed)
    ixy1,std1= (ny/2-0.5,nx/2-0.5), 3.2
    for k in range(nSample):
        x0= rg.standard_normal(size=max_cells*2)
        y0= rg.standard_normal(size=max_cells*2)

        y1,x1= np.rint(std1*y0+ixy1[0]).astype(int), np.rint(std1*x0+ixy1[1]).astype(int)
        y1[y1>=ny]-=ny
        x1[x1>=nx]-=nx
        #y1,x1= y1[y1>=0], x1[x1>=0]
        ct=i=0
        while(ct<=max_cells):
            if arr2[k,y1[i],x1[i]]==0:
                arr2[k,y1[i],x1[i]]=1
                ct+=1
            i+=1

    ### Case3: Two Gaussian normal distributions on corners
    arr3= np.zeros([nSample,ny,nx])
    rg= np.random.default_rng(rg_seed)
    ixy1,std1= (ny*0.25-0.5,nx*0.25-0.5), 2.
    ixy2,std2= (ny*0.75-0.5,nx*0.75-0.5), 2.4
    ...
    for k in range(nSample):
        ...
        ct=i=0
        y1,x1= np.rint(std1*y0+ixy1[0]).astype(int), np.rint(std1*x0+ixy1[1]).astype(int)
        y2,x2= np.rint(std2*x0+ixy2[0]).astype(int), np.rint(std2*y0+ixy2[1]).astype(int)
        y1= y1[np.logical_and(y1>=0, y1<ny)]
        y2= y2[np.logical_and(y2>=0, y2<ny)]
        x1= x1[np.logical_and(x1>=0, x1<nx)]
        x2= x2[np.logical_and(x2>=0, x2<nx)]
    ...
    ois= []
    for arr in arrs:
        ois.append(com.identify_aggregate_and_get_org_indices(arr,diag=False))
    
    ###----
    ### Make a plot
    fig= plt.figure()
    fig.set_size_inches(11,8.5) #(8.5,9)
    ...
    npnx,gapx,npny,gapy= 4, 0.04, 2.8, 0.
    ...
    fnout = './Pics/Fig09_random_example.20x20.tgt_den{}.{}.png'.format(tgt_den,var_nm)
